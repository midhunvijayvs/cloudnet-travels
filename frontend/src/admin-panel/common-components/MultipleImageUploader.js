...
const MultipleImageUploader = ({ state, setterFunction, setLoading, formFrrors,imgGalleryToggle, setFormErrors, restaurantId }) => {
  ...
  const loadData = () => {
    ...
    if (window.localStorage.getItem('itemSelectedId') !== null) {
      setLoading(true);
      API.get(`/restaurants/restaurant-images/?restaurant_id=${window.localStorage.getItem("itemSelectedId")}`)
        .then(response => {
          // Handle the response
    ...
    if (state.length < 10) {
      ...
      img.onload = () => {
        ...
        if (!allowedTypes.includes(file.type)) {
          ...
          setLoading(true);
          API.post("/restaurants/restaurant-images/", formData, {
            headers: {
  ...
  const handleRemove = (id) => {
    ...
    setLoading(true);
    API.delete(`/restaurants/restaurant-images/${id}/`)
      .then(response => {
        // Handle the response
