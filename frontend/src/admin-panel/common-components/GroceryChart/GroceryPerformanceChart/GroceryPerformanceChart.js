...
import './GroceryPerformanceChart.scss'



const GroceryPerformanceChart = ({ source }) => {
  ...
  const loadData = () => {
    ...
    // Performance Data::
    let apiUrl2 = `/grocery/other-grocery-store-averages/`;
    // Loop through the filters object and append selected filters to the apiUrl
    for (let filter in filters) {
      if (filters[filter]) {
...
export default GroceryPerformanceChart;
