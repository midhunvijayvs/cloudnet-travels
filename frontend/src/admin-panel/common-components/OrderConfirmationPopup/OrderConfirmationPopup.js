...
import './OrderConfirmationPopup.scss'
import API from '../../../API';
...
const OrderConfirmationPopup = ({ mode, setterFunction, selectedItem, loadData, loadTableData, setTableData }) => {
  ...
  const loadOrderData = () => {
    let apiUrl = `/order/orders/${selectedItem.id}/`;
    setIsLoading(true)
    API.get(apiUrl)
      .then(response => {
        setIsLoading(false)
  ...
  return (
    <div className='custom-modal order-confirm-popup'>
      ...
      {/* assign */}
      {isAssigneeModalOpen &&
        <OrderDriversPopup setterFunction={setIsAssigneeModalOpen} data={data} loadData={loadOrderData} loadTableData={loadTableData}
      ...
      {isOrderModifyModalOpen &&
        <OrderModifyPopup setterFunction={setIsOrderModifyModalOpen} data={data} loadData={loadOrderData}
