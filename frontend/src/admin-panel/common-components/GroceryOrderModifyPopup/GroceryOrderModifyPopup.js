...
import './GroceryOrderModifyPopup.scss'
import API from '../../../API';
...
const GroceryOrderModifyPopup = ({ setterFunction, data, loadData, loadTableData, setTableData, orderConfirmPopup }) => {
  ...
  const addAlternative = (newItem, variant) => {
    ...
    const newItemObject = {
      ...
      final: true,
      grocery_item: newItem.id,
      grocery_item_details: newItem,
      grocery_item_name: newItem?.name,
  ...
  const updateOrderItems = async () => {
    const url = "/grocery/order-item/";
    const orderUrl = `/grocery/orders/${data.id}/update-delivery-status/`;
    ...
    for (const item of orderData.items) {
      if (item.id && existingItemsMap.has(item.id)) {
        ...
        if (JSON.stringify(existingItem) !== JSON.stringify(item)) {
          console.log(item.grocery_item_name, '...updating');
          hasChanges = true;
              ...
              if (item.decision === "accepted") {
                ...
                for (const altItem of existingItemsMap.values()) {
                  if (
                    ...
                    console.log(altItem.grocery_item_name, '...deleting alternative item');
                    deletePromises.push(
        ...
        console.log(item.grocery_item_name, '...adding');
        hasChanges = true;
  ...
  const loadMenuData = () => {
    let apiUrl = `/grocery/grocery-items/?page_size=100&is_available=True&is_published=true`;
    if (ItemSearchInput) {
      apiUrl += `&search_key=${ItemSearchInput}`;
    }
    if (data.restaurant) {
      apiUrl += `&grocery_store=${data.restaurant}`;
    }
    setIsLoading(true)
    API.get(apiUrl)
      .then(response => {
...
export default GroceryOrderModifyPopup;