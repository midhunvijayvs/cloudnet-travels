...
const CuisineList = () => {
  ...
  const deleteItem = () => {
    setIsLoading(true)
    API.delete(`/restaurants/cuisines/${selectedItem.id}`)
      .then(response => {
  ...
  const loadTableData = () => {
    ...
    let apiUrl = `/restaurants/cuisines/?page=${page}&page_size=${pageSize}`;
  ...
  const save = () => {
    setIsLoading(true)
    if (mode == "add") {
      API.post('/restaurants/cuisines/', { name: selectedItem.name })
        .then((response) => {
          setSelectedItem({});
          showFormPopup(false)
          loadTableData();
    ...
    setIsLoading(true)
      API.put(`/restaurants/cuisines/${selectedItem.id}/`, { name: selectedItem.name })
        .then(response => {
          setSelectedItem({});
          showFormPopup(false)
          setIsLoading(false)
                        ...
                        <table className="table category-table" id="table_id">
                          <thead>
                            <tr>
                              ...
                              <th className="text-center">Cuisine Name</th>
                              {/* <th>Date</th> */}
                              <th>Option</th>
                            </tr>
                          </thead>
                          ...
                          <tbody>
                            {data && data.results && data.results.map((cuisine) => (
                              <tr>
                                {/* <td>
                                  <input className="custom-checkbox" type="checkbox" name="text" />
                                </td> */}
                                <td className="text-center">{cuisine.name}</td>
                                {/* <td>26-12-2021</td> */}
                                <td>
                                  <ul
                                    ...
                                    <li>
                                      <button onClick={() => { setMode("edit"); showFormPopup(true); setSelectedItem(cuisine) }}>
                                        <i className="ri-pencil-line"></i>
                                      </button>
                                    </li>

                                    <li>
                                      <button className="" onClick={() => { setSelectedItem(cuisine); setIsDeleteConfModalOpen(true) }}>
            ...
            <div className='main'>
              ...
              <form>
                <div className='mb-2'>
                  ...
                  <div><input className='inp-F0F0F0 w-100 p-2 fw-500 mb-4' placeholder='Add Cuisine Name'
                    name='cuisine_name'
